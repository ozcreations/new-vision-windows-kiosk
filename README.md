...
- **Imagery analysis stalled:** The kiosk loads Street View imagery through the local proxy and inspects it with the browser Canvas API. Ensure the Google key has Street View Static API enabled and that the kiosk is served from the same origin so the image can be analyzed client-side.

## Data sources & methodology

- **Street imagery:** Google Street View Static API imagery is fetched through the kiosk proxy and analyzed in-browser with a lightweight edge-detection heuristic to estimate façade window bands and floors.
